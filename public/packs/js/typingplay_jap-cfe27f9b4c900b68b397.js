  ...
  function typingPlayStart() {
    ...
    function keyDownCharCountUp(keyDownChar) {
      ...
      if (charArrayInDisplayWordJap[0].textContent === "っ") {
        if (keyDownChar.key !== "a" && keyDownChar.key !== "i" && keyDownChar.key !== "u" && keyTempForSmallTsuSecond !== "t" && keyDownChar.key !== "e" && keyDownChar.key !== "o" && keyDownChar.key !== "n") {
          // アルファベットの入力を開始していない場合
          if (shiftAlreadyStartedCountAll === 0) {
            keyTempForSmallTsuPreFirst = keyDownChar.key;
            shiftAlreadyStartedCountAll = 1; // 1アルファベット以上入力済みの場合
          } else {
            // 2文字目が存在する場合(最後の文字ではない場合)
            ...
            console.log("test??"); // "っ"の単独入力を可能にする
            ...
            } else if (keyTempForSmallTsuSecond === "t" && keyDownChar.key === "s") {
              keyTempForSmallTsuPreThird = keyDownChar.key;
              correctTyping();
            } else if (keyTempForSmallTsuSecond === "t" && keyDownChar.key === "u") {
              keyTempForSmallTsuPreFirst = "";
...
/******/ });
//# sourceMappingURL=typingplay_jap-cfe27f9b4c900b68b397.js.map