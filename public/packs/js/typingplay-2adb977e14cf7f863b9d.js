...
window.addEventListener('load', function () {
  ...
  document.getElementById("meterImg").src = imgArray[bonusCountMeter]; // 終了を知らせるポップアップを定義する。

  var closePopup = document.getElementById("closePopup"); // let endFlag = "0";
  // 表示する単語を作成する処理を定義する。
  ...
  function createDisplayWord() {
    ...
    charArrayInDisplayWord = wordList[randomIntegerInTotalWordList].split("").map(function (value) {
      ...
      displayWord.appendChild(spanValue); // スコアアップの基準値として使用する変数を定義する。

      charArrayInDisplayWordOrg.textContent = charArrayInDisplayWord;
      return spanValue;
    });
  ...
  function countUpScore() {
    console.log(charArrayInDisplayWordOrg);

    if (charArrayInDisplayWordOrg.length < refTextLengthForCountUpLevel1) {
...
/******/ });
//# sourceMappingURL=typingplay-2adb977e14cf7f863b9d.js.map