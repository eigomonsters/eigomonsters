...
window.addEventListener('load', function () {
  // もう一度遊ぶを実施した際に、ゲーム開始までのカウントダウンとメーターを表示させる処理の判定フラグ
  var reStartFlag = "0";
  gameStart(reStartFlag);
});
...
function gameStart() {
  ...
  var timerIdToStart = setInterval(countDownTimeToStart, 1000); // もう一度遊ぶを実施した際に、ゲーム開始までのカウントダウンとメーターを表示させる処理

  if (reStartFlag == "1") {
    var displayReStartCountdownAndMeter = function displayReStartCountdownAndMeter() {
    ...
    displayReStartCountdownAndMeter();
...
/******/ });
//# sourceMappingURL=typingplay_eng-3f36e0e17f377979f5ac.js.map