...
function gameStart() {
  ...
  function typingPlayStart() {
    ...
    function createDisplayWord() {
      ...
      if (playEndFlag === "0") {
        ...
        if (questionOrderMode == "番号順") {
          ...
          var randomIntegerInTotalWordList = Math.floor(Math.random() * wordList.length);
          console.log(randomIntegerInTotalWordList); // 表示する単語をランダムで選定する。

          displayWordInList = wordList[randomIntegerInTotalWordList];
          console.log(displayWordInList);
        }

        ; // 同じ問題を２度出題しないように、一度出題した問題はwordListから削除する。
        // wordList.length = 0になった場合
        // プレイモード：練習・普通の場合は、wordListを初期値に戻す。
        // プレイモード：タイムアタックの場合は、ゲームを終了する。

        console.log(wordList.length);

        if (wordList.length = 0) {
          if (playMode == "練習" || playMode == "普通") {
            wordList = (_readOnlyError("wordList"), gon.wordListAll);
          } else {
            // フラグを1に更新する。
        ...
        ;
        console.log("0になってません"); // 英単語を生成した際に、発音させる。（発音ありの場合のみ）
    ...
    function countDownTime() {
      ...
      var remainSecLabel = "\u6B8B\u308A".concat("".concat(remainMin).slice(-1), "\u5206").concat("0".concat(remainSec).slice(-2), "\u79D2"); // 残り時間が0になった場合の処理を定義する。
      ...
      setTimeout(function () {
        ...
        if (playMode == "練習" || playMode == "普通") {
          ...
          if (gameCompleteTimeAttackFlag == "1") {
            closePopup.textContent = "\u3042\u306A\u305F\u304C\u304B\u304B\u3063\u305F\u6642\u9593\u306F \u5206 \u79D2\u3067\u3059!";
            closePopup.className = "endResultPopup";
          } else {
...
/******/ });
//# sourceMappingURL=typingplay_jap-b4fc115d77cab364a469.js.map