      ...
      if (playEndFlag === "0") {
        ...
        if (playMode == "普通" || playMode == "正確重視") {
          ...
          if (duplicateWords.length >= 1) {
            console.log(duplicateWords.length); // 重複している英単語の配列を定義する。

            var duplicateWordsList = [];

            for (var i = 0; i <= duplicateWords.length; i++) {
              console.log(duplicateWords[i]);
              console.log(duplicateWords[i].jap_trans_1);
...
/******/ });
//# sourceMappingURL=typingplay_eng-dbc8bb6a3913c19bd8f3.js.map