  ...
  function typingPlayStart() {
    ...
    function createDisplayWord() {
      ...
      if (playEndFlag === "0") {
        ...
        allQuestionWords.push(eachQuestionOrder, displayWordInList.id, displayWordInList.eng_word, displayWordInList.jap_trans_1, "未解答", "-"); //表示する単語の発音させる。（発音ありの場合のみ）
    ...
    function gameClose() {
      ...
      var allQuestionsListScr = document.getElementById("allQuestionsListScr");
      var allQuestionsList = document.getElementById("allQuestionsList");

      for (var i = 0; i < allQuestionWordsList.length; i++) {
        // 配列の中のオブジェクトの数だけ処理を繰り返す
        var tr = document.createElement("tr");
        allQuestionsList.appendChild(tr); // 1行の中を生成

        var objArray = Object.entries(allQuestionWordsList[i]); // オブジェクトを配列に

        for (var j = 0; j < objArray.length; j++) {
          var td = document.createElement("td");
          td.textContent = objArray[j]; // arr[1]はvalueの部分
      ...
      ; // for (let i = 0; i < allQuestionWordsList.length; i++){
      // 	let eachQuestionWords = allQuestionWordsList[i];
      // 	for (let j = 0; j < allQuestionWords.length; j++){
      // 		allQuestionsList.innerHTML = `<li>${eachQuestionWords[j]}</li>`; // リストを入れる。
      // 	};
      // };

      var allQuestionsListBorder = document.getElementById("allQuestionsListBorder");
      ...
      checkQuestions.onclick = function () {
        ...
        allQuestionsListScr.className = "allQuestionsListScr";
        allQuestionsListBorder.className = "allQuestionsListBorder";
      ...
      closeAllQuestionsList.onclick = function () {
        ...
        allQuestionsListScr.className = "transparent";
        allQuestionsListBorder.className = "transparent";
    ...
    function correctTyping() {
      ...
      if (charArrayInDisplayWord.length === 0) {
        ...
        allQuestionWords.splice(allQuestionWords.length - 1, 1, "".concat("0".concat(eachQuestionAnswerMin).slice(-2), "\u5206").concat("0".concat(eachQuestionAnswerSec).slice(-2), "\u79D2")); // 出題したすべての英単語リストに追加する。

        allQuestionWordsList.push(allQuestionWords);
        console.log(allQuestionWordsList); // 丸を表示、答えを表示するなどのアニメーションを実施する。
...
/******/ });
//# sourceMappingURL=typingplay_eng-165fe7a49be54d8e6777.js.map