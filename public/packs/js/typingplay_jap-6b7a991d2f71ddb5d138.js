    ...
    function keyDownCharCountUp(keyDownChar) {
      ...
      if (shiftAlreadyStartedCountAll === 0) {
        ...
        if (charArrayInDisplayWordJap[1] != null) {
          ...
          var _wordLetterWithSmallLetter = charArrayInDisplayWordJap[0].textContent + charArrayInDisplayWordJap[1].textContent; // 文字をローマ字に変換し、アルファベットごとに分ける。
          // 2文字に対して、ローマ字変換パターン１をsplitする。


          if (romaMappingTableFirst[_wordLetterWithSmallLetter] != null) {
            charArrayInDisplayWordAlphaFirst = romaMappingTableFirst[_wordLetterWithSmallLetter].split("").map(function (value) {
          ...
          if (romaMappingTableSecond[_wordLetterWithSmallLetter] != null) {
            charArrayInDisplayWordAlphaSecond = romaMappingTableSecond[_wordLetterWithSmallLetter].split("").map(function (value) {
              var spanValue = document.createElement("span");
              spanValue.textContent = value;
              return spanValue;
            });
          ...
          if (romaMappingTableThird[_wordLetterWithSmallLetter] != null) {
            charArrayInDisplayWordAlphaThird = romaMappingTableThird[_wordLetterWithSmallLetter].split("").map(function (value) {
              var spanValue = document.createElement("span");
              spanValue.textContent = value;
              return spanValue;
            });
        ...
        var _wordLetterSole = charArrayInDisplayWordJap[0].textContent; // 1文字目のみ対して、ローマ字変換パターン１をsplitする。
        // 母音の場合はsplit不可のため処理を分ける。

        if (_wordLetterSole !== "あ" && _wordLetterSole !== "い" && _wordLetterSole !== "う" && _wordLetterSole !== "え" && _wordLetterSole !== "お") {
          charArrayInDisplayWordAlphaSole = romaMappingTableFirst[_wordLetterSole].split("").map(function (value) {
            var spanValue = document.createElement("span");
            spanValue.textContent = value;
            return spanValue;
          });
          ...
          var spanValue = document.createElement("span");
          spanValue.textContent = romaMappingTableFirst[_wordLetterSole];
          charArrayInDisplayWordAlphaSole = [spanValue];
        }

        ;
        console.log(charArrayInDisplayWordAlphaSole); // 1文字目のみ対してローマ字変換パターン２が存在する場合はsplitする。

        if (romaMappingTableSecond[_wordLetterSole] != null) {
          charArrayInDisplayWordAlphaSoleSecond = romaMappingTableSecond[_wordLetterSole].split("").map(function (value) {
            var spanValue = document.createElement("span");
            spanValue.textContent = value;
            return spanValue;
          });
        ...
        if (romaMappingTableThird[_wordLetterSole] != null) {
          charArrayInDisplayWordAlphaSoleThird = romaMappingTableThird[_wordLetterSole].split("").map(function (value) {
            var spanValue = document.createElement("span");
            spanValue.textContent = value;
            return spanValue;
          });
...
/******/ });
//# sourceMappingURL=typingplay_jap-6b7a991d2f71ddb5d138.js.map