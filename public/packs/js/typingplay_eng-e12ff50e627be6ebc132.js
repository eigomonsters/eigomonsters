      ...
      if (playEndFlag === "0") {
        ...
        if (playMode == "普通" || playMode == "正確重視") {
          ...
          if (duplicateWords.length >= 1) {
            console.log(duplicateWords); // 重複している英単語の配列を定義する。

            var duplicateWordsList = [];

            for (var i = 0; i < duplicateWords.length; i++) {
              console.log(i);
              console.log(duplicateWords[i]);
              console.log(displayWordInList.id);
              var duplicateWordCandidate = duplicateWords[i];
              console.log(duplicateWordCandidate); // 重複している英単語のみを配列duplicateWordsListに格納する。
              //if (duplicateWords[i].id == displayWordInList.id){

              duplicateWordsList.push(duplicateWordCandidate.eng_word); //}
            }
...
/******/ });
//# sourceMappingURL=typingplay_eng-e12ff50e627be6ebc132.js.map