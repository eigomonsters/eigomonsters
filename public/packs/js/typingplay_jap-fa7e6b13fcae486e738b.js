...
var agent = window.navigator.userAgent.toLowerCase(); // スペースかEnterキーを押下するとゲーム開始のカウントダウンがスタートする。
...
function gameStart() {
  ...
  if (bgmMode == "BGMあり") {
    var _bgmAudio = new Audio('/assets/bgm1.mp3');

    var baseVol = 0.3;
    var fadeSpeed = 100;
    _bgmAudio.volume = 0.1;

    _bgmAudio.play();

    var start_func = setInterval(function () {
      _bgmAudio.volume = _bgmAudio.volume + baseVol / 100;

      if (_bgmAudio.volume >= baseVol - baseVol / 100) {
        _bgmAudio.volume = baseVol;
  ...
  if (agent.indexOf("safari") != -1 && bgmMode == "BGMなし" && pronounceMode == "発音あり") {
    var _bgmAudio2 = new Audio('/assets/bgm1.mp3');
  ...
  function typingPlayStart() {
    ...
    function gameClose() {
      ...
      clearInterval(timerId); // bgmありの場合、bgmを止める。
...
/******/ });
//# sourceMappingURL=typingplay_jap-fa7e6b13fcae486e738b.js.map