...
window.addEventListener('load', function () {
  ...
  // const wordList = gon.wordlist[0].eng_word;
  ...
  function typingPlayStart() {
    ...
    function createDisplayWord() {
      ...
      var randomIntegerInTotalWordList = Math.floor(Math.random() * gon.wordList[0].length); // 表示する単語を一文字ごとに区切り、spanタグで閉じる。
      // spanタグで閉じた単語を表示する。

      charArrayInDisplayWord = gon.wordList[randomIntegerInTotalWordList].eng_word.split("").map(function (value) {
        // spanタグを作成する。
...
/******/ });
//# sourceMappingURL=typingplay-ff019ca7cb8ec8e4bb13.js.map