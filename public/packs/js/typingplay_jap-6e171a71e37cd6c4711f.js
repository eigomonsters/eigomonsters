...
function gameStart() {
  ...
  var bonusAddPlayTime = 3000; // 英単語を順番に出題する場合の番号の初期値を定義する。

  var _sortById = 0;
  console.log("初期値設定"); // メーターの初期値を表示する。
  ...
  function typingPlayStart() {
    ...
    var createDisplayWordOkFlagFromBonusCountReset = "0";
    console.log(_sortById); // 表示する単語を作成する。

    createDisplayWord(_sortById); // プレイ開始時間を取得する。
    ...
    var initialPlayingTimeMSec = 50000; // スタート時に残りのプレイ可能時間の初期値を表示するためにcountDownTimeを実施する。
    ...
    function createDisplayWord(_sortById) {
      ...
      if (playEndFlag === "0") {
        ...
        console.log(_sortById); // 英単語を順番に出題する場合の番号の初期値を定義する。

        displayWordInList = wordList[_sortById];
        _sortById++;
        console.log(_sortById); // // sortByIdが出題英単語数を超えた場合、初期値に戻す。
    ...
    function correctTyping() {
      ...
      if (charArrayInDisplayWordJap.length === 0) {
        ...
        console.log(_sortById);
        setTimeout(createDisplayWord(_sortById), 200);
        setTimeout(function () {
    ...
    function bonusCountReset() {
      if (playMode == "正確重視" && createDisplayWordOkFlagFromBonusCountReset == "0") {
        ...
        setTimeout(createDisplayWord(_sortById), 200); //createDisplayWord処理実行後にcreateDisplayWordOkFlagFromBonusCountReset==0に戻す。
...
/******/ });
//# sourceMappingURL=typingplay_jap-6e171a71e37cd6c4711f.js.map