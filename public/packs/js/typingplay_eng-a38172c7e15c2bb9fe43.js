...
function gameStart() {
  ...
  function typingPlayStart() {
    ...
    var initialPlayingTimeMSec = 30000; // スタート時に残りのプレイ可能時間の初期値を表示するためにcountDownTimeを実施する。
    ...
    }; // "ヒントを見る"ボタンを表示する。(プレイモード普通、正確重視の時のみ)


    if (playMode == "普通" || playMode == "正確重視") {
      var _hintButton = document.getElementById("hintButton");

      _hintButton.innerHTML = "ヒントを見る";
      _hintButton.className = "hintButton";

      _hintButton.onclick = function () {
    ...
    restartButtonMidGame.innerHTML = "最初からやり直す";
    restartButtonMidGame.className = "restartButtonMidGame"; // "コース選択画面に戻る"ボタンを表示する。

    var courseSettingBackButtonMidGame = document.getElementById("courseSettingBackButtonMidGame");
    courseSettingBackButtonMidGame.innerHTML = "コース選択画面に戻る";
    courseSettingBackButtonMidGame.className = "courseSettingBackButtonMidGame"; // 表示するスコアを定義する。
    ...
    function countDownTime() {
      ...
      if (remainMSec <= 0) {
        ...
        pronounceAgainButton.className = "transparent"; // "ヒントを見る"ボタンを消す。（プレイモード普通、正確重視の時のみ）

        if (playMode == "普通" || playMode == "正確重視") {
          hintButton.className = "transparent";
        }

        ; // "答えを見る"ボタンを消す。（プレイモード普通、正確重視の時のみ）
        // また、注釈も消す。
...
/******/ });
//# sourceMappingURL=typingplay_eng-a38172c7e15c2bb9fe43.js.map