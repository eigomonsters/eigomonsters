...
function gameStart() {
  ...
  function typingPlayStart() {
    ...
    var playEndFlag = "0"; // 英単語を順番に出題する場合の番号の初期値を定義する。

    var sortById = 0; // createDisplayWord処理を実行して問題ないかを判定するフラグを用意する。
    // bonusCountReset処理で200m秒内に連続で処理が発生しないようにするため。

    var createDisplayWordOkFlagFromBonusCountReset = "0"; // 表示する単語を作成する。

    createDisplayWord(sortById); // プレイ開始時間を取得する。
    ...
    function createDisplayWord(_sortById) {
      ...
      if (playEndFlag === "0") {
        ...
        var randomIntegerInTotalWordList = Math.floor(Math.random() * 2); // 表示する単語をランダムで選定する。

        displayWordInList = wordList[_sortById];
        console.log(wordList.length); // 英単語を順番に出題する場合の番号の初期値を定義する。
        // displayWordInList = wordList[_sortById];
        // let _sortById = _sortById + 1;
    ...
    function correctTyping() {
      ...
      if (charArrayInDisplayWordJap.length === 0) {
        ...
        setTimeout(createDisplayWord(sortById), 200);
        setTimeout(function () {
    ...
    function bonusCountReset() {
      if (playMode == "正確重視" && createDisplayWordOkFlagFromBonusCountReset == "0") {
        ...
        setTimeout(createDisplayWord(sortById), 200); //createDisplayWord処理実行後にcreateDisplayWordOkFlagFromBonusCountReset==0に戻す。
...
/******/ });
//# sourceMappingURL=typingplay_jap-a755a0900bb6d05d3010.js.map