...
var questionOrderMode = gon.questionOrderMode;
...
function gameStart() {
  ...
  function typingPlayStart() {
    ...
    var initialPlayingTimeMSec = 1000000; // スタート時に残りのプレイ可能時間の初期値を表示するためにcountDownTimeを実施する。
    ...
    courseSettingBackButtonMidGame.classList.remove('transparent'); // 表示するスコアを定義する。
    ...
    function countDownTime() {
      ...
      if (remainMSec <= 0) {
        ...
        pronounceAgainButton.className = "transparent"; // ヒントを見るボタンを消す。（プレイモード普通、正確重視の時のみ）

        hintButton.className = "transparent"; // 答えを見るボタンを消す。（プレイモード普通、正確重視の時のみ）
    ...
    function correctTyping() {
      ...
      keyboardAudio.volume = 0.3; //keyboardAudio.play();
      // メーターを更新する。
    ...
    function bonusCountReset() {
      ...
      if (playMode == "正確重視" && createDisplayWordOkFlagFromBonusCountReset == "0") {
        ...
        setTimeout(createDisplayWord, 500); //createDisplayWord処理実行後にcreateDisplayWordOkFlagFromBonusCountReset==0に戻す。
...
/******/ });
//# sourceMappingURL=typingplay_jap-1ca4d574e66e512605cc.js.map