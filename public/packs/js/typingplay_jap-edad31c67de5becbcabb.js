    ...
    function createDisplayWord() {
      ...
      if (playEndFlag === "0") {
        ...
        if (questionOrderMode == "番号順") {
          ...
          var _randomIntegerInTotalWordList = Math.floor(Math.random() * wordList.length); // 表示する単語をランダムで選定する。


          displayWordInList = wordList[_randomIntegerInTotalWordList];
        }

        ; // 同じ問題を２度出題しないように、一度出題した問題はwordListから削除する。

        wordList.splice(randomIntegerInTotalWordList, 1); // wordList.length = 0になった場合
...
/******/ });
//# sourceMappingURL=typingplay_jap-edad31c67de5becbcabb.js.map