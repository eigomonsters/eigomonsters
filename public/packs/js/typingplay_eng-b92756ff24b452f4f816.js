...
function gameStart() {
  ...
  function typingPlayStart() {
    ...
    var totalPlayingTimeMSec = 0; // 問題を作成した時間を定義する。

    var eachQuestionCreateTime = 0; // 問題を解答するのに要した時間を定義する。

    var eachQuestionAnswerTime = 0; // スタート時に残りのプレイ可能時間の初期値を表示するためにcountDownTimeを実施する。
    ...
    function gameClose() {
      ...
      for (var i = 0; i < allQuestionWordsList.length; i++) {
        var eachQuestionWords = allQuestionWordsList[i];

        for (var j = 0; j < allQuestionWords.length; j++) {
          allQuestionsList.innerHTML = "<li>".concat(eachQuestionWords[j], "</li>"); // リストを入れる。
        }
...
/******/ });
//# sourceMappingURL=typingplay_eng-b92756ff24b452f4f816.js.map