...
window.addEventListener('load', function () {
  ...
  var timerId = setInterval(countDownTime, 1000); // 現在のスコアを定義する。

  var currentScore = document.getElementById("currentScore");
  currentScore.textContent = 0; // 単語の長さによってcountUpするScoreを変更する処理を定義する。

  var refTextLengthForCountUpScoreLevel1 = 4;
  var addScoreLevel1 = 1;
  var addScoreLevel2 = 3; // 
  // setTimeout(countPoint,0);

  function countUpScore() {
    if (charArrayInDisplayWord.length < refTextLengthForCountUpScoreLevel1) {
      addPoint = addScoreLevel1;
    } else {
      addPoint = addScoreLevel2;
    }

    currentScore = currentScore + addPoint;
    return currentScore;
  ...
  function keyDownCharCountUp(keyDownChar) {
    if (keyDownChar.key === charArrayInDisplayWord[0].textContent) {
      ...
      if (charArrayInDisplayWord.length === 0) createDisplayWord(), countUpScore(), console.log(currentScore);
...
/******/ });
//# sourceMappingURL=typingplay-63bad7647ca9de4bd109.js.map