...
window.addEventListener('load', function () {
  ...
  var romaMappingTable = (_romaMappingTable = {
    ...
    "きょ": "kyo",
    "じゅ": "zyu"
  }, _defineProperty(_romaMappingTable, "\u3058\u3085", "ju"), _defineProperty(_romaMappingTable, "え", "e"), _romaMappingTable); // 表示するメーター初期値を定義する。
  ...
  function typingPlayStart() {
    ...
    var initialPlayingTimeMSec = 20000; // スタート時に残りのプレイ可能時間の初期値を表示するためにcountDownTimeを実施する。
    ...
    var displayWordAlpha = []; // タイプした文字が表示文字と一致する場合の処理を定義する。

    function keyDownCharCountUp(keyDownChar) {
      if (charArrayInDisplayWordJap[1].textContent !== "" || charArrayInDisplayWordJap[1].textContent === "ぁ" || charArrayInDisplayWordJap[1].textContent === "ぃ" || charArrayInDisplayWordJap[1].textContent === "ぅ" || charArrayInDisplayWordJap[1].textContent === "ぇ" || charArrayInDisplayWordJap[1].textContent === "ぉ" || charArrayInDisplayWordJap[1].textContent === "ゃ" || charArrayInDisplayWordJap[1].textContent === "ゅ" || charArrayInDisplayWordJap[1].textContent === "ょ" || charArrayInDisplayWordJap[1].textContent === "っ") {
        var wordLetter = charArrayInDisplayWordJap[0].textContent + charArrayInDisplayWordJap[1].textContent;
        console.log(wordLetter);

        if (charArrayInDisplayWordAlpha[0] = null) {
          charArrayInDisplayWordAlpha = romaMappingTable[wordLetter].split("").map(function (value) {
            // spanタグを作成する。
            var spanValue = document.createElement("span"); // spanタグに一文字を挿入する。

            spanValue.textContent = value; // // 表示する単語にspanタグで閉じた一文字を追加する。
            // displayWordAlpha.appendChild(spanValue);

            return spanValue;
          });

          if (keyDownChar.key === charArrayInDisplayWordAlpha[0].textContent) {
            charArrayInDisplayWordAlpha[0].textContent = charArrayAlpha;
            charArrayInDisplayWordAlpha.shift();
          }
        }

        console.log(charArrayInDisplayWordAlpha);
      } else {// let wordLetter = charArrayInDisplayWordJap[0].textContent
      } // if(keyDownChar.key === charArrayInDisplayWordAlpha[0].textContent && playEndFlag==="0") {
      //   // 文字色を変更する。
...
/******/ });
//# sourceMappingURL=typingplay_jap-8b2e956f39426be42b8b.js.map