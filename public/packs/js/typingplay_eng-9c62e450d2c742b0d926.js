  ...
  function typingPlayStart() {
    ...
    function createDisplayWord() {
      ...
      if (playEndFlag === "0") {
        ...
        displayWordInList = wordList[randomIntegerInTotalWordList]; //表示する単語の発音させる。（プレイモードが練習の時のみ）
    ...
    function correctTyping() {
      ...
      if (charArrayInDisplayWord.length === 0) {
        // 正解音を定義する。
        var correctAudio = new Audio('/assets/正解音.mp3'); // 正解した場合、効果音を発生させる。
    ...
    function bonusCountReset() {
      // 不正解音を定義する。
      var incorrectAudio = new Audio('/assets/不正解音.mp3'); // 不正解の場合、効果音を発生させる。
    ...
    ; // 英単語を生成した際に、発音させる処理を定義する。（プレイモードが練習の場合のみ）
...
/******/ });
//# sourceMappingURL=typingplay_eng-9c62e450d2c742b0d926.js.map