...
window.addEventListener('load', function () {
  ...
  function typingPlayStart() {
    ...
    function keyDownCharCountUp(keyDownChar) {
      ...
      if (charArrayInDisplayWordJap[1] != null) {
        ...
        if (charArrayInDisplayWordJap[1].textContent === "ぁ" || charArrayInDisplayWordJap[1].textContent === "ぃ" || charArrayInDisplayWordJap[1].textContent === "ぅ" || charArrayInDisplayWordJap[1].textContent === "ぇ" || charArrayInDisplayWordJap[1].textContent === "ぉ" || charArrayInDisplayWordJap[1].textContent === "ゃ" || charArrayInDisplayWordJap[1].textContent === "ゅ" || charArrayInDisplayWordJap[1].textContent === "ょ") {
          ...
          if (shiftAlreadyStartedFlag === 0) {
            charArraySplitInRomaWithSmallLetter(); // 1文字目のみ対して、ローマ字変換パターン１をsplitする。
          ...
          if (shiftAlreadyStartedFlag === 0) {
            if (keyDownChar.key === romaMappingTableFirst[_wordLetter]) {
              charArrayInDisplayWordJap[0].className = "add_red";
              charArrayInDisplayWordJap.shift();
              correctTyping();
            } else {
          ...
          if (shiftAlreadyStartedFlag === 0) {
            // ローマ字変換パターン１をsplitする。
            charArrayInDisplayWordAlphaFirst = romaMappingTableFirst[_wordLetter2].split("").map(function (value) {
              var spanValue = document.createElement("span");
              spanValue.textContent = value;
              return spanValue;
        ...
        if (shiftAlreadyStartedFlag === 0) {
          if (keyDownChar.key === romaMappingTableFirst[_wordLetter3]) {
            charArrayInDisplayWordJap[0].className = "add_red";
            charArrayInDisplayWordJap.shift();
            correctTyping();
          } else {
        ...
        if (shiftAlreadyStartedFlag === 0) {
          // ローマ字変換パターン１をsplitする。
          charArrayInDisplayWordAlphaFirst = romaMappingTableFirst[_wordLetter4].split("").map(function (value) {
            var spanValue = document.createElement("span");
            spanValue.textContent = value;
            return spanValue;
          }); // ローマ字変換パターン２が存在する場合はsplitする。
      ...
      // 2文字目が小文字("っ"を除く)場合の処理を定義する。

      function charArraySplitInRomaWithSmallLetter() {
        var wordLetterWithSmallLetter = charArrayInDisplayWordJap[0].textContent + charArrayInDisplayWordJap[1].textContent; // 2文字に対して、ローマ字変換パターン１をsplitする。
...
/******/ });
//# sourceMappingURL=typingplay_jap-0dc32b0324c1afe00a3a.js.map