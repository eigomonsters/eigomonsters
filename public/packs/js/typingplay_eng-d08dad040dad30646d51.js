...
function gameStart() {
  ...
  function typingPlayStart() {
    ...
    var initialPlayingTimeMSec = 6000; // プレイモード：タイムアタックの場合は30分とする。
    ...
    function correctTyping() {
      ...
      if (charArrayInDisplayWord.length === 0) {
        ...
        allQuestionWordsList.push(allQuestionWords); // 合計正解数、連続正解数、最大連続正解数をカウントアップする+1を定義。

        displayAddCorrectNum.textContent = "+1";
        displayAddConsecutiveCorrectNum.textContent = "+1";
        displayAddMaxConsecutiveCorrectNum.textContent = "+1"; // 合計正解数、連続正解数、最大連続正解数+1をフェードアウトする。

        displayAddCorrectNum.classList.add('fadeout');
        displayAddConsecutiveCorrectNum.classList.add('fadeout');
        displayAddMaxConsecutiveCorrectNum.classList.add('fadeout'); // fadeoutのclassを取り除くことで、2回目以降も延長した残りプレイ時間が表示されるようにする。
        ...
        }, 2000); // 丸を表示、答えを表示するなどのアニメーションを実施する。
...
/******/ });
//# sourceMappingURL=typingplay_eng-d08dad040dad30646d51.js.map